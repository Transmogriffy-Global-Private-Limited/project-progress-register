...
- `internal/projects` — project lifecycle, temporal membership, geofence versions, task ownership/plural responsibility and before/after revisions, append-only Admin assessments, authorized dashboard/timeline read models, scoped PostgreSQL queries, and audit orchestration.
...
- Tasks live inside the project aggregate. PostgreSQL project locks keep membership stable during task operations; Admins may edit any task while Members require both current access and immutable creator ownership. Zero or more responsible Members are display/work assignments only and never grant access or ownership. V1 preserves its singular transport during migration; V2 exposes the complete set over the same authoritative rows.
...
- The filestore module streams allowlisted bytes to a private staging root, calculates SHA-256, and atomically finalizes opaque keys. PostgreSQL attachment state remains authoritative; immediate-and-minute reconciliation recovers pending finalization without blocking process liveness. Authorized video reads reuse the same private file handle through HTTP byte-range streaming; no media server or public storage URL is introduced.
