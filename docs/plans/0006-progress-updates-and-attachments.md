...
- Verification is per attachment. Only an image reported as an in-Chrome camera capture may be `verified`, and only when its upload location passes server-side accuracy and geofence evaluation. Gallery/existing-file images, every document, and every video are always `non_verified` with an explicit reason while retaining the same geotag.
...
- Original filenames are display metadata only. Opaque random storage keys determine filesystem paths. Downloads require current project authorization and use attachment disposition plus `nosniff`.
