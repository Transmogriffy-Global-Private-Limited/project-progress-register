...
- Every Admin or current project Member may read a complete oldest-first task timeline. It combines task before/after revisions, progress/revisions, attachments and state/access events, comments, accepted suggestions, and assessment versions with event-specific metadata.
...
- Audit cursors are opaque, stable across equal timestamps, and rejected as validation errors when malformed.
...
- Timeline reads use the same current Admin/member project scope as the task, preserve chronological ordering, and retain enough immutable metadata to recreate each state transition.
...
- Backup and restore do not print database credentials, expose listeners, or silently overwrite existing state.
...
The neutral dashboard, complete paginated audit, authorized task timeline, migration `000007`, OpenAPI/human contracts, focused tests, extended live verifier, coordinated backup/restore scripts, and operator documentation are implemented. Automated verification passes; database-live migrations/lifecycle and a disposable restore drill remain required for full verification.
