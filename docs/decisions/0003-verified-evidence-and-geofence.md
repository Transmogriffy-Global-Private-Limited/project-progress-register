...
Progress updates need mobile camera photographs and videos, current browser location, accuracy, site geofence evaluation, and a server-authoritative time. Browser data is useful but cannot attest hardware truth or prevent a compromised client from spoofing input.
...
Verification is attachment-specific. Only an image or video reported as an in-Chrome direct camera capture is eligible, and only when reported accuracy is within policy and the uncertainty circle is inside the geofence. Existing/gallery images and videos plus documents are explicitly non-verified while remaining geotagged. Geofence failure affects classification, not submission acceptance.
...
- The conservative uncertainty rule may classify direct camera photo/video media non-verified when its centre is inside but accuracy overlaps the boundary.
