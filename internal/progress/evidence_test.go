...
func TestAttachmentVerificationRequiresDirectCameraImageOrVideoAndVerifiedLocation(t *testing.T) {
	...
	for _, kind := range []string{"image", "video"} {
		status, _ := attachmentVerification(kind, "camera", evidence.LocationStatus)
		if status != "verified" {
			t.Fatalf("%s/camera status=%s", kind, status)
		}
	}
	for _, test := range []struct{ kind, source string }{{"image", "upload"}, {"document", "upload"}, {"document", "camera"}, {"video", "upload"}} {
...
func TestEvidencePreservesOutsideLocation(t *testing.T) {
	...
	for _, kind := range []string{"image", "video"} {
		status, _ := attachmentVerification(kind, "camera", evidence.LocationStatus)
		if status != "non_verified" {
			t.Fatalf("outside %s/camera status=%s", kind, status)
		}
	}
}
